...
| GET | `/matters` | List/filter matters. Filters: `applicant`, `facility`, `matter_type`, `action_type`, `county`, `service_area`, `bed_count_min/max`, `year_filed`, `final_outcome`, `highest_review_level`, `service_type`, `phase`, `q`. Paging: `limit` (default 50, max 500), `offset`; `sort` (prefix `-` for DESC) | `curl 'localhost:8000/matters?county=Fulton&service_type=PET&sort=-year_filed'` |
...
| GET | `/documents` | List/filter documents. Filters: `docket_id`, `doc_type`, `decision_level`, `phase`, `outcome`, `validation_status`, `decision_maker`, `doc_date_from/to`, `template_name`, `ocr_status`, `duplicate_of`, `q` | `curl 'localhost:8000/documents?doc_type=Order&doc_date_from=2024-01-01'` |
